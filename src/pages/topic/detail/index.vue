        ...
        <v-content style="bottom: 0;" v-vuet-scroll="{ path: 'topic-detail', name: 'content' }">
            ...
            <template v-if="!data.loading && data.existence">
                ...
                <ul class="re-list">
                    ...
                    <li flex="box:first">
                        <div class="headimg">
                            <router-link class="pic"
                                         :to="{ name: 'user-detail', params: { username: author.loginname } }"
                                         :style="{ backgroundImage: 'url(' + author.avatar_url + ')' }"></router-link>
                        </div>
                        <div class="bd">
                            <div flex>
                                <router-link flex-box="0"
                                             :to="{ name: 'user-detail', params: { username: author.loginname } }">{{
                                    author.loginname }}
                                </router-link>
                    ...
                    <li v-for="(item, $index) in replies">
                        <div flex="box:first">
                            <div class="headimg">
                                <router-link class="pic"
                                             :to="{ name: 'user-detail', params: { username: item.author.loginname } }"
                                             :style="{ backgroundImage: 'url(' + item.author.avatar_url + ')' }"></router-link>
                            </div>
                            <div class="bd">
                                <div flex>
                                    <router-link flex-box="0"
                                ...
                                <div class="bottom" flex="dir:right cross:center">
                                    ...
                                    <div class="icon" :class="{ fabulous: testThing(item.ups) }"
                                         v-if="item.author.loginname !== self.loginname" @click="fabulousItem(item)">
